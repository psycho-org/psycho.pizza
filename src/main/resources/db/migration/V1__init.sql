...
idx_refresh_tokens_account_id
on refresh_tokens using btree (account_id);

-- refresh_tokens jti 유니크 인덱스
create unique index if not exists
uk18kw8ppjw4gmmgdns9ecyg17b
on refresh_tokens using btree (
    jti
);

-- refresh_tokens token_hash 유니크 인덱스
create unique index if not exists
uko2mlirhldriil2y7krapq4frt
on refresh_tokens using btree (
    token_hash
);
...
create unique index if not exists
uk_memberships_account_workspace_active
on memberships using btree (
